import { initQueue } from "@extra/queues/vehicle";
...
export default async function handler(req, res) {
  ...
  upload(req, res, async function (err) {
    ...
    // Add CSV processing job to pg-boss queue
    const boss = await initQueue();
    await boss.send("vehicle", { filePath: url, companyId: session?.companyId, userId: session?.id });
  });
