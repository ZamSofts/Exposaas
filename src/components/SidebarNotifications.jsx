...
const SidebarNotifications = ({ isCollapsed = false }) => {
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      const WS_URL = process.env.NOTIFICATION_WS_URL || 'ws://localhost:5001';
    ...
    <div className="relative">
      ...
      <button
        ...
        className={`relative p-2 rounded-lg transition-all duration-200 border ${
          isConnected 
            ? 'text-[var(--foreground)] hover:bg-[var(--accent)] border-[var(--border)]' 
            : 'text-[var(--muted-foreground)] cursor-not-allowed border-red-300'
        }`}
      ...
      {showNotifications && (
        <div 
          className="sidebar-notification-dropdown absolute top-0 w-66 max-h-96 bg-[var(--surface)] border border-[var(--border)] rounded-lg shadow-xl overflow-hidden z-5000"
          style={{
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-[var(--foreground)] font-semibold text-xs leading-tight">
                        {notification.title}
                      </h4>
    ...
    : 'No message available.'}
                    ...
                    {!notification.read && (
                      <button
                        ...
                        className="text-xs text-[var(--primary)] hover:text-[var(--primary)]/70 flex-shrink-0"
                      >
                        ✓
                      </button>
                    )}
