...
export default function Payments({ vehicleId }) {
  ...
  // File preview states
  const [previewFile, setPreviewFile] = useState(null); // For storing existing document info in edit mode
  const [showPreview, setShowPreview] = useState(null); // For showing preview modal
  ...
  useEffect(() => {
    const handleClickOutside = event => {
      ...
      if (modalRef.current && !modalRef.current.contains(event.target) && !showPreview) {
        resetForm();
      }
    };
  ...
  }, [showPreview]); // Add showPreview as dependency
  ...
  const handleFileChange = e => {
    ...
    if (!allowedExtensions.includes(fileExtension) || !allowedTypes.includes(file.type)) {
      setError("Invalid file type. Only PDF, JPG, PNG, DOC, DOCX files are allowed.");
  ...
  const editData = async () => {
    ...
    try {
      ...
      if (edit === 0) {
        ...
        response = await API("PUT", "vehiclePayments", formData, true);
      } else {
        setCustomLoader(true);
        formData.append("id", edit);
        response = await API("POST", "vehiclePayments", formData, true);
      }
      ...
      showToast(
        edit === 0 ? `Payment created successfully${response.fileUploaded ? " with document" : ""}!` : `Payment updated successfully${response.fileUploaded ? " with new document" : ""}!`,
        "success"
      );
  ...
  const loadEdit = async id => {
    ...
    try {
      ...
      // Handle existing document URL properly
      if (data.url) {
        setPreviewFile({
          url: data.url,
          fileName: data.url.split("/").pop() || "Document",
        });
        setDocumentFile(null); // Clear any new file selection
      } else {
        setPreviewFile(null);
  ...
  // Calculate stats
  const totalAmount = payments.reduce((sum, payment) => sum + (parseFloat(payment.name.replace(/[^\d.]/g, "")) || 0), 0);
  ...
  return (
    <div className="relative">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div ref={modalRef} className="bg-[var(--surface)] border bounce border-[var(--border)] rounded-xl p-6 w-full max-w-2xl">
            ...
            <div className="space-y-6">
              {/* First Row: Payment Name and Date */}
                  ...
                  <div className="border border-[var(--border)] rounded-lg bg-[var(--surface)] p-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-shrink-0">
                        {documentFile ? (
                          /* New File Preview */
                          <div className="w-16 h-16 border border-[var(--border)] rounded-lg overflow-hidden bg-[var(--input)] flex items-center justify-center">
                            {documentFile.type && documentFile.type.includes("image") ? (
                              <img src={URL.createObjectURL(documentFile)} alt={documentFile.name} className="w-full h-full object-cover" />
                            ) : documentFile.type === "application/pdf" ? (
                              <div className="text-red-500">
                                <FileUp className="w-6 h-6" />
                              </div>
                            ) : (
                              <FileUp className="w-6 h-6 text-[var(--primary)]" />
                            )}
                          </div>
                        ) : (
                          ...
                          <div className="w-16 h-16 border border-[var(--border)] rounded-lg overflow-hidden bg-[var(--input)] flex items-center justify-center">
                            ...
                            (previewFile.fileName.toLowerCase().includes(".jpg") || previewFile.fileName.toLowerCase().includes(".jpeg") || previewFile.fileName.toLowerCase().includes(".png")) ? (
                              <img
                                src={previewFile.url}
                              ...
                              />
                            ) : previewFile && previewFile.fileName && previewFile.fileName.toLowerCase().includes(".pdf") ? (
                              <div className="text-red-500">
                                <FileUp className="w-6 h-6" />
                              </div>
                            ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium text-[var(--foreground)] truncate">{documentFile ? documentFile.name : previewFile ? previewFile.fileName : "Document"}</p>
                            <p className="text-xs text-[var(--secondary-foreground)] mt-1">{documentFile ? `${(documentFile.size / 1024).toFixed(1)} KB` : "Existing document"}</p>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              onClick={() =>
                                setShowPreview({
                                  url: documentFile ? URL.createObjectURL(documentFile) : previewFile.url,
                                  fileName: documentFile ? documentFile.name : previewFile.fileName,
                                })
                              }
      ...
      {/* File Preview Modal */}
      {showPreview && <FilePreviewer url={showPreview.url} fileName={showPreview.fileName} isOpen={true} onClose={() => setShowPreview(null)} trigger={null} />}
