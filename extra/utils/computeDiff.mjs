...
export function computeDetailedDiff(originalJson, correctedJson) {
  ...
  const diff = { vehicles: [], totalFieldsCompared: 0, totalFieldsChanged: 0 };

  if (origVehicles.length !== corrVehicles.length) {
    const minLen = Math.min(origVehicles.length, corrVehicles.length);
    diff.totalFieldsCompared = minLen * COMPARED_FIELDS.length;
  ...
  for (let i = 0; i < origVehicles.length; i++) {
    ...
    const vDiff = { index: i, fields: {}, charges: { added: [], removed: [], changed: [] } };
...
export function computeScoredDiff(extractedVehicles, goldenJson) {
  ...
  if (extractedVehicles.length !== goldVehicles.length) {
    return {
      ...
      fieldsChanged: 1,
      totalFieldsCompared: Math.min(extractedVehicles.length, goldVehicles.length) * COMPARED_FIELDS.length,
    };
  }
